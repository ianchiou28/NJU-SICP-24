...
class Ant(Insect):
    ...
    def store_ant(self, other):
        if self.can_contain(other):
            self.stored_ant = other
        else:
            raise AssertionError("Cannot store this ant")
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if self.can_contain(place.ant):
                ...
                raise AssertionError(f"Two ants in {place}")
            # END PROBLEM 8
    ...
    def buff(self):
        ...
        # BEGIN PROBLEM 12
        if not hasattr(self, 'damage_doubled'):
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health <= 0 and current_place:
            bees = list(current_place.bees)
            for bee in bees:
...
class ContainerAnt(Ant):
    ...
    def store_ant(self, ant):
        # BEGIN PROBLEM 8
        assert self.can_contain(ant), f"{self} cannot contain {ant}"
        self.ant_contained = ant
        # END PROBLEM 8

    def remove_ant(self, ant):
        if self.ant_contained is not ant:
            raise ValueError(f"{self} does not contain {ant}")
        self.ant_contained = None
...
class QueenAnt(ScubaThrower):
    def __init__(self, health=1):
        super().__init__(health)  # You should change this line
    ...
    def construct(cls, gamestate):
        ...
        # BEGIN PROBLEM 12
        if cls.food_cost > gamestate.food:
            print('Not enough food remains to place QueenAnt')
            return None
        if hasattr(gamestate, 'has_queen') and gamestate.has_queen:
            return None
    ...
    def action(self, gamestate):
        ...
        while place:
            if place.ant:
                if not hasattr(place.ant, 'doubled_damage_by_queen'):
                    place.ant.damage *= 2
                    place.ant.doubled_damage_by_queen = True
                if place.ant.is_container and place.ant.ant_contained:
                    contained_ant = place.ant.ant_contained
                    if not hasattr(contained_ant, 'doubled_damage_by_queen'):
                        contained_ant.damage *= 2
                        contained_ant.doubled_damage_by_queen = True
            place = place.exit
        # END PROBLEM 12
    def zero_health_callback(self):
