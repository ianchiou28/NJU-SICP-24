...
class shiftcipher(methods):
    ...
    def encrypt(self, message):
        ...
        codes = []
        for c in message:
            if (ord(c)+self.shift)>=97 and (ord(c)+self.shift)<=122:
                codes.append(ord(c)+self.shift)
            elif (ord(c)+self.shift)>122:
                codes.append(ord(c)+self.shift-26)
            elif (ord(c)+self.shift)<97:
                codes.append(ord(c)+self.shift+26)
...
class dictionarycipher(methods):
    ...
    def encrypt(self, message):
        ...
        for i in range(len(chars)):
            if self.dictionary.get(chars[i]):
                results.append(self.dictionary.get(chars[i]))
            else:
...
class fencecipher(methods):
    ...
    def encrypt(self, message):
        ...
        groups = [message[i:i + self.rails] for i in range(0, len(message), self.rails)]
        ...
        for col, group in enumerate(groups):
            for row, char in enumerate(group):
                grid[row][col] = char
...
class encryption:
    ...
    def encrypt(self, message):
        ...
        return self.extention.decorator(self.method.encrypt, message)
...
class TACard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        self.other_card.attack, self.other_card.defense = self.other_card.defense, self.other_card.attack
