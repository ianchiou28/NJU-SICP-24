...
def about(topic):
    ...
    def contains_topic(paragraph):
        words = split(remove_punctuation(paragraph.lower()))
...
def minimum_mewtations(typed, source, limit):
    ...
    if limit <0:
        return limit+1
    if  len(typed)==0 or len(source)==0 or limit<0:  # Fill in the condition
        # BEGIN
        if limit<0:
            return limit+1
        else:
        ...
        return min_cost if min_cost <= limit else limit + 1
        # END
    # END PROBLEM 7
...
def final_diff(typed, source, limit):
    ...
    def helper(t_idx, s_idx, remaining_limit):
        if (t_idx, s_idx, remaining_limit) in memo:
            return memo[(t_idx, s_idx, remaining_limit)]
        if remaining_limit < 0:
            return float('inf')
        if t_idx == len(typed):
            return len(source) - s_idx
        if s_idx == len(source):
            return len(typed) - t_idx
        if typed[t_idx] == source[s_idx]:
            cost = helper(t_idx + 1, s_idx + 1, remaining_limit)
        else:
            add_cost = 1 + helper(t_idx, s_idx + 1, remaining_limit - 1)
            remove_cost = 1 + helper(t_idx + 1, s_idx, remaining_limit - 1)
            substitute_cost = 1 + helper(t_idx + 1, s_idx + 1, remaining_limit - 1)
            cost = min(add_cost, remove_cost, substitute_cost)
        memo[(t_idx, s_idx, remaining_limit)] = cost
        return cost
    result = helper(0, 0, limit)
...
def report_progress(typed, prompt, user_id, upload):
    ...
    # BEGIN PROBLEM 8
    correct_words = 0
    for t_word, p_word in zip(typed, prompt):
        if t_word == p_word:
            correct_words += 1
        else:
            break
...
def time_per_word(words, times_per_player):
    ...
    for player_times in times_per_player:
        player_word_times = [
            player_times[i] - player_times[i - 1] for i in range(1, len(player_times))
        ]
...
def fastest_words(game):
    ...
    # BEGIN PROBLEM 10
    fastest_words_by_player = [[] for _ in player_indices]
    for word_index in word_indices:
        min_time = float('inf')
        fastest_player = None
        for player in player_indices:
            time_taken = time(game, player, word_index)
            if time_taken < min_time:
                min_time = time_taken
                fastest_player = player
        fastest_words_by_player[fastest_player].append(get_word(game, word_index))

    return fastest_words_by_player
    # END PROBLEM 10
